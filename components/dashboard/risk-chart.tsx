...
const seriesConfig = [
  { key: 'flood', label: 'Flood Risk', color: 'var(--color-chart-1)' },
  { key: 'infrastructure', label: 'Infrastructure Risk', color: 'var(--color-chart-2)' },
  { key: 'population', label: 'Population Risk', color: 'var(--color-chart-3)' },
]
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-md border border-border bg-popover px-2.5 py-2 shadow-xl">
      <div className="mb-1 font-mono text-[11px] font-semibold text-foreground">{label}</div>
      {payload.map((p: any) => (
    ...
    >
      <div className={compact ? 'h-48' : 'h-56'}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={riskTrajectory} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
            <defs>
              {seriesConfig.map((s) => (
            ...
            <ReferenceArea x1="NOW" x2="+60m" fill="var(--color-primary)" fillOpacity={0.05} />
            <ReferenceLine
              x="NOW"
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 border-t border-border pt-3">
        ...
        <div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Predicted Peak</div>
